import { Pane, Heading, IconButton, Text } from 'evergreen-ui';
...
export const CombinedSummary: React.FC<ICombinedSummaryProps> = props => {
  const { accounts, endBalances } = props;
  ...
  return (
    <div ref={containerRef} style={{ display: 'flex', marginTop: '20px' }}>
      <Pane
        ...
        maxWidth={nameWidth}
      >
        <Pane borderBottom height={67} width='100%'></Pane>
        {accounts.map(ac => {
        ...
        <Pane borderTop borderRight borderBottom borderRadius={0}>
          ...
          >
            {endBalances.map((eb, index) => {
              return (
                <Pane key={index} minWidth={displayWidth} borderLeft borderRadius={0}>
                  <Pane
                    display='flex'
                    alignItems='center'
                    ...
                    height={31}
                  >
                    <Heading>{formatDateMonthYear(eb.date)}</Heading>
                  </Pane>
                  <Pane minHeight={400}>

...
import { allMonthsBetweenDates, getEarliestDate, getLatestDate, getMonthAndYearFromDate, stringToString } from '../../../utils/date.util';
...
const mapStateToProps = (state: IStore, { accountId }: OwnProps): StateProps => {
  ...
  const monthAndYears = recordDates
    ? allMonthsBetweenDates(getEarliestDate(recordDates), getLatestDate(recordDates))
      .map(my => getMonthAndYearFromDate(my))
    : [];

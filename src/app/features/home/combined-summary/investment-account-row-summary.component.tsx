...
const selectAccountBalances = (query: DateCurriedQuery, dates: string[]) =>
    createSelector(AccountSelectors.account, InvestmentRecordSelectors.recordsByAccountId, (account, records) => {
    ...
    if (rates && rates.length === dates.length) {
        combinedBalances = balances.map((b, i) => {
            ...
            return {
                ...
                total:
                    (b?.cadTotal || 0.0) +
                    (!isNullOrUndefined(b?.usdTotal) ? b?.usdTotal * parseFloat(rates[i].rate) : 0.0),
            };
        });
